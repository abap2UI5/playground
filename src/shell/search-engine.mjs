 ...
 *
 * Four documents carry that box: the playground, the sample catalogue, the
 * per-sample pages and the documentation. Three of them are these static
 * documents and one is a Vue application, so anything shared between them has
 * to be plain JavaScript with no imports. This file is that, and
 * `docs/.vitepress/theme/search-engine.js` in abap2UI5/docs is where it is
 * maintained — this is the copy, kept in step by hand, the same arrangement as
 * the palette and site-memory. Change one, change the other.
 *
 * What is NOT copied is the index it reads: that is one generated document at
 * /docs/search-index.json, built over there from the pages of the manual and
 * the three sample catalogues, and fetched by whichever site the reader is on.
...
export function rememberQuery(query) {
  const q = (query || "").trim();
  try {
    if (!q || q.length > QUERY_MAX) globalThis.localStorage?.removeItem(QUERY_KEY);
    else globalThis.localStorage?.setItem(QUERY_KEY, JSON.stringify({ q, at: Date.now() }));
  } catch {
...
export function recallQuery() {
  let record = null;
  try {
    record = JSON.parse(globalThis.localStorage?.getItem(QUERY_KEY) || "null");
  } catch {
