...
//                    (src/editor/transpile-core.mjs)
//
// This module runs in a dedicated worker (src/editor/registry-worker.mjs),
// and the page talks to it through src/editor/registry.mjs, which exports
// the same names with a promise in front of each. Nothing here knows about
// the page: no DOM, no Monaco, only abaplint - which is also what lets
// tools/check-size.mjs run the same parse under Node.
//
...
// The check alone, for the page to ask before it commits to a reparse.
export const validateSettings = (next) => validated(next);
...
// rename, prepareRename, documentFormatting, codeActions, implementation,
// documentHighlight, semanticTokensRange. All of them take and return plain
// LSP objects, which is what lets them cross the thread to the page's Monaco
// providers (src/editor/providers.mjs) as messages.
...
// rather than a sort.
let corpusNames;
export function corpusObjectNames() {
...
export function knownObjectNames() {
  const own = userObjects()
    ...
    .map((o) => ({ name: o.getName(), type: o.getType() }))
    .sort(byName);
  return merge(own, corpusObjectNames());
}
