...
`npm run build` is five cached steps and takes minutes the first time; after
...
`tests/samples.spec.js` imports the list of samples the page carries and drives every entry, so
